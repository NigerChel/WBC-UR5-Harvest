...
//////////////////////////////////////////////////////////
  ...
  // Retrieve the generalized position and velocity, and joint torques
   ...
   for (std::size_t j = 0; j < msg.joints.size(); ++j) {
    ...
    Eigen::VectorXd         ub;
		...
		int dof_act = model.nv-6; //DoF of the number of actuated part
    ...
    qpmad::MatrixIndex num_ctr = 6 + dof_act + dof_act + c*m + 3*c; // Number of constraints that will be taken in the QP plus 6*c to take into acccount the rigid contacts
		...
		//Read YAML file
	  YAML::Node conf = YAML::LoadFile("/home/niger/ros_ws/src/wbc_ur5/config/param_position_ur5_joint_limit_mod.yaml"); 
		/// Weights for QP ///
    ...
    S_act_ddq.block(0, 6, dof_act, dof_act).setIdentity();
  ...
  ///////////////////////////////////////////////////////////////////////////////////////////////// 
	...
	Eigen::MatrixXd J4 = Eigen::MatrixXd::Zero(model.nv - 6, model.nv); 
	Eigen::MatrixXd S4 = Eigen::MatrixXd::Zero(6, model.nv - 6);
...
//////////////////////////////////////////////////////////////////7
    ...
    block_4.setIdentity(m*c,m*c);
		...
		////////////////////////////////////////////////////////////////////////////////////////////
		 H.block(0, 0, dof, dof) = (2.0*gamma1*block_1) + (2.0*w1*J1.transpose()*S1.transpose()*S1*J1) + (2.0*w4*J4.transpose()*S4.transpose()*S4*J4);
		///////////////////////////////////////////////////////////////////////////////////////////

 H.block(0, dof, dof, m * c) = block_2;
    H.block(dof, 0, m * c, dof) = block_3;
    H.block(dof, dof, m * c, m * c) = (2.0*gamma2)*block_4;
		
		// Giving values of the gradient h
    h.setRandom(size);
    
    ///////////////////////////////////////////////////////////////////////////////////////////////
      h.block(0, 0, dof, 1) =  - 2.0*w1*J1.transpose()*S1.transpose()*S1*d1 - 2.0*w4*J4.transpose()*S4.transpose()*S4*d4;
    //////////////////////////////////////////////////////////////////////////////////////////////


	h2.setConstant(m*c,1,0);
 	h.block(dof, 0, m * c, 1)= h2; 
 ...
 ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    lb.resize(size);
    ub.resize(size);
    lb.head(dof).setConstant(-10);
    lb.tail(4).setConstant(0);
    ub.head(dof).setConstant(10);
    ub.tail(4).setConstant(10);

	// Giving values of A from data.Ag, Q1, Q2 blocks 
    A.resize(num_ctr, size);
    A.setZero();
    // Setting the values for equality constraints
    A.block(0,0,6,dof) = Eigen::MatrixXd::Zero(6,12); //data.Ag; //data.dAg
    A.block(0,dof,6,m*c) =  Eigen::MatrixXd::Zero(6,4); //-Q1;
    // Setting the values for inequality constraints for joint position q
    A.block(6,0,dof_act,dof) = S_act_ddq;
    A.block(6,dof,dof_act,4) = Eigen::MatrixXd::Zero(dof_act,4);
		// Setting the values for inequality constraints for joint velocity dq
		A.block(dof,0,dof_act,dof) = S_act_ddq;
    A.block(dof,dof,dof_act,m*c) = Eigen::MatrixXd::Zero(dof_act,m*c);
		// Setting the values for inequality constraints for force intensities rho
    A.block(dof+dof_act,dof,4,4) = Eigen::MatrixXd::Zero(4,4); Eigen::MatrixXd::Identity(4,4);
		// Taking into account rigid contacts 
		A.block(dof+dof_act+4,0,3,dof) =  Eigen::MatrixXd::Zero(3,12); //J1.block(0,0,3,dof);
...
// Setting values for the inequality constrains //
    ...
    Alb.block(0,0,6,1) = Eigen::VectorXd::Constant(6,0.01); //Wg - momenta_drift;
    Alb.block(6,0,dof_act,1) = (2.0/dt*dt) * (lb_bound - q.tail(dof_act) - v.tail(dof_act)*dt);
    Alb.block(dof_act+6,0,dof_act,1) = (1.0/dt) * (lb_bound_dq - v.tail(dof_act));
    Alb.block(dof_act+dof_act+6,0,4,1) = Eigen::VectorXd::Constant(4,0.0);
    // Lower bound contact inequality
    Alb.block(dof_act+dof_act+6+4,0,3,1) = Eigen::VectorXd::Constant(3,0.0); //-a_drift_leg_left.linear();

		
    Aub.block(0,0,6,1) =  Eigen::VectorXd::Constant(6,0.01); // Wg - momenta_drift;
    Aub.block(6,0,dof_act,1) = (2.0/dt*dt) * (ub_bound - q.tail(dof_act) - v.tail(dof_act)*dt);
    Aub.block(dof_act+6,0,dof_act,1) = (1.0/dt) * (ub_bound_dq - v.tail(dof_act));
    Aub.block(dof_act+dof_act+6,0,4,1) = Eigen::VectorXd::Constant(4,0.0);
    // Upper bound contact inequality
    Aub.block(dof_act+dof_act+6+4,0,3,1) = Eigen::VectorXd::Constant(3,0.0); //-a_drift_leg_left.linear();
    
    /////////////////////////////////////////////////////////
    lb.block(6,0,dof_act,1) = (2.0/dt*dt) * (lb_bound - q.tail(dof_act) - v.tail(dof_act)*dt);
	 ub.block(6,0,dof_act,1) = (2.0/dt*dt) * (ub_bound - q.tail(dof_act) - v.tail(dof_act)*dt);
 ...
 qpmad::Solver solver;
	...
	Eigen::VectorXd dq_des = v.tail(dof_act) + a_des.tail(dof_act)*dt; //+ a_des.tail(30)*(dt*dt/2)
   	Eigen::VectorXd q_d = q.tail(dof_act) + dq_des*dt;
