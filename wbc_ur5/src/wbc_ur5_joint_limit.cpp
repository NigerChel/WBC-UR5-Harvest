  ...
  // Retrieve the generalized position and velocity, and joint torques
   ...
   for (std::size_t j = 0; j < msg.joints.size(); ++j) {
    ...
    qpmad::MatrixIndex num_ctr = 6 + dof_act + dof_act + c*m + 3*c; // Number of constraints that will be taken in the QP plus 6*c to take into acccount the rigid contacts
		...
		//Read YAML file
	  YAML::Node conf = YAML::LoadFile("/home/niger/ros_ws/src/wbc_ur5/config/param_position_ur5_joint_limit.yaml"); 
		/// Weights for QP ///
...
//////////////////////////////////////////////////////////////////7
 ...
 ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    A.block(dof+dof_act,dof,4,4) = Eigen::MatrixXd::Identity(4,4);
		// Taking into account rigid contacts 
		A.block(dof+dof_act+4,0,3,dof) = J1.block(0,0,3,dof);
...
// Setting values for the inequality constrains //
    ...
    Alb.block(dof_act+dof_act+6+4,0,3,1) = -a_drift_leg_left.linear();
    ...
    Aub.block(dof_act+dof_act+6,0,4,1) = Eigen::VectorXd::Constant(4,10.0);
    // Upper bound contact inequality
    Aub.block(dof_act+dof_act+6+4,0,3,1) = -a_drift_leg_left.linear();
