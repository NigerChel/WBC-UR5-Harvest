  ...
  // Retrieve the generalized position and velocity, and joint torques
   ...
   for (std::size_t j = 0; j < msg.joints.size(); ++j) {
    ...
    qpmad::MatrixIndex num_ctr = 6 + dof_act + dof_act + c*m + 3*c; // Number of constraints that will be taken in the QP plus 6*c to take into acccount the rigid contacts
		...
		//Read YAML file
	  YAML::Node conf = YAML::LoadFile("/home/niger/ros_ws/src/wbc_ur5/config/real_param_position_ur5_param_joint_limit.yaml"); 
		/// Weights for QP ///
    ...
    S_act_ddq.block(0, 6, 1, 1).setIdentity();
...
/*    A.block(0,0,6,dof) = data.Ag; //data.dAg
 ...
 ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    lb.resize(size);
    ub.resize(size);
    lb.head(dof).setConstant(-10000);
    lb.tail(4).setConstant(0);
    ub.head(dof).setConstant(10000);
...
// Setting values for the inequality constrains //
  ...
  /*  Alb.block(0,0,6,1) = Wg - momenta_drift;
    ...
    Aub.block(dof_act+dof_act+6,0,4,1) = Eigen::VectorXd::Constant(4,1000000.0);
 ...
 ////////////////////////////////////////////////////////////////////////////// 
    ...
    double distance_first_component = std::abs(ub_bound(0) - q(7));
