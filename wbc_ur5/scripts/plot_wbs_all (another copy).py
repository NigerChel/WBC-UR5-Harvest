...
def save_data_to_csv():
    """Save the collected joint positions and time data to a CSV file."""
    ...
    with open(csv_file_path, mode='w') as file:
        ...
        # Write the header
        header = ['Time (s)'] + ['Joint {} Position'.format(i + 1) for i in range(6)]
        writer.writerow(header)

        # Ensure the length of both time_data and joint_positions are the same
        max_len = max(len(time_data_reemc), len(time_data_whole_body))

        for i in range(max_len):
            # For each row, write the time and positions from reemc
            row = [time_data_reemc[i] if i < len(time_data_reemc) else '']
            for j in range(6):
                row.append(joint_positions_data_reemc[j][i] if i < len(joint_positions_data_reemc[j]) else '')
            writer.writerow(row)
